...
└── HelloWorld_IOS/                              # the only project (was a Hello World template)
    ...
    │   ├── SettingsViewModel.cs                 # APS creds form + VerboseLogging toggle (session-only, in-memory)
...
9. **Navigation event logging**: button-press handlers emit `apsDiag` messages (`nav.view: …`, `nav.bake: …`, `nav.theme: …`, `nav.tree-panel: …`). Gated host-side by the **Verbose logging** Settings toggle — which as of 2026-07-02 is **session-only**: a plain in-memory flag (`Logger.VerboseLogging`), default OFF at every launch, applied instantly when the switch flips (no Save), never persisted. It was previously stored in Preferences/NSUserDefaults, which survives reinstalls — combined with the Settings form's `= true` field initializer being persisted by any credentials Save, the toggle came back ON in every new build. The bridge gates both `nav.*` and `vrb.*` prefixed diag lines on it; everything else always logs. iOS memory warnings are logged always-on under `app.memory` (wired in `AppDelegate.FinishedLaunching` via `DidReceiveMemoryWarningNotification`) — they timestamp the pressure build-up that precedes each WebView content-process kill.
...
11. **APS viewer memory trims + crash-loop breaker**: the 2026-07-02 test showed a 476 MB federated NWD getting the WebView content process killed by iOS every ~40 s during navigation, which looped patch #10's auto-reload indefinitely. Two-part fix: (a) *shrink the footprint* — `loadDocumentNode(..., { skipExternalIds: true })`, plus `setQualityLevel(false,false)`, ground shadow/reflection off, progressive rendering + optimize-navigation on, and the GL renderer's pixel ratio capped at 1.5 on retina (DPR 2 = 4× framebuffer pixels); (b) *stop the loop* — `ViewerPage` tracks terminations, and ≥3 within 2 min suspends model auto-reload (tabs are recreated empty), logs `app.lifecycle` ERROR, and shows a "Model too heavy" alert with Retry.
    **History:** the first version of this patch used `skipPropertyDb: true` (drops the whole in-wasm property DB — we read element properties via the Model Derivative REST API host-side anyway). That stopped the kills but broke the viewer's built-in **Model Browser**, which builds its tree from the property DB: the tree panel spun on "Loading" forever, on models of every size. `skipExternalIds` keeps the property DB (tree works) but drops the external-ID table — one long Navisworks path string per element, the bulk of the DB's wasm footprint for federated NWDs. Whether the remaining property DB fits the 476 MB reference model inside the iPad's WebView memory ceiling is pending an on-device test; if kills return, next fallback is a host-side tree fetched from `GET /modelderivative/v2/designdata/{urn}/metadata/{guid}`.

12. **Render-stall watchdog (APS tabs)**: 2026-07-02 follow-up field report — with patch #11's trims the process kills stopped, but pan/zoom stutters, then the canvas goes **white** and stays white until the user presses the Home (view) button. The session log showed no `webglcontextlost`, no process termination, and the model still loaded: the render loop had simply stopped presenting frames (progressive rendering + optimize-navigation defer the full frame during movement; under load it never lands, and nothing re-triggers a render once the camera stops).
    **v2 (same day, after 2nd field test — 20 logged recoveries but the user still hit a freeze that only the Home tap fixed):** the v1 watchdog only ran while camera events flowed, so a canvas that came up blank *at rest* (e.g. right after a crash-recovery reload) was never caught. Now a **deadline model**: whenever a frame is *expected* — camera moving (rolling now+1.2 s) or a model just finished loading (now+5 s) — `apsWdDeadline` is armed; a presented frame at rest disarms it (the viewer legitimately renders nothing when idle, so "no frames while idle" alone is NOT a stall). A single always-running 600 ms interval kicks `impl.invalidate(true,true,true)` when the deadline passes and re-arms every 2 s until a frame lands.
    **Kick classification (22:07 field log):** a kick fires ~1 s after *every* move stop — even on a 15-fragment model — because the last presented frame re-arms the deadline and a static scene legitimately renders nothing further. That guaranteed post-nav redraw is *wanted* (it's the mechanism that cured the white-out), but logging it as a stall drowned real stalls. Kicks where no frame landed since the last camera move log always-on as `render stall detected — forced invalidate (#N, navigating|idle)` (1–20 then every 10th); kicks where a frame *did* land log verbose-only as `vrb.watchdog: post-nav refresh (#N)`. Tab close logs `watchdog totals this session: stalls=N · post-nav refreshes=M`. `vrb.camera: move start/stop` transitions are verbose-gated. Also wired always-on **load milestones**: `geometry loaded in Ns · fragments=N`, `object tree (property db) ready in Ns`, `object tree UNAVAILABLE`, and verbose-gated `vrb.load-progress` ticks at 25 % steps — the ticks are gated on `!apsGeomLoaded` (set by `GEOMETRY_LOADED_EVENT`, reset per load) because after the initial stream `PROGRESS_UPDATE_EVENT` flaps 0 %↔100 % on every progressive re-render pass (~500 spam lines in the 21:45 field log).
    **Validated by the 2026-07-02 21:45 field test:** a full ~6-min session on the 476 MB model with ZERO crashes; 21 watchdog kicks, the `idle` ones firing ~1.2 s after each `camera: move stop` — exactly the freeze that previously needed a Home-button tap. Model stats from the milestones: property DB ready in 106.5 s, geometry fully streamed in 184.4 s, 2,761,112 fragments — navigation is at its worst during the first ~3 min while geometry still streams.

13. **APS double-load + lifetime fixes**: the 2026-07-02 20:43 session log showed `pre-load`/`post-load` **twice** right after a crash-recovery reload — `RehydrateTabs` sends `loadAps` (which switches tabs internally and starts loading) then `switchTab` for the active tab, whose JS handler started a *second* concurrent load of the same 476 MB model at the worst possible moment. Three fixes in `viewer.html`: (a) `switchTab` is now idempotent (`tabId === activeTabId` early-out); (b) `loadApsTab` has an in-flight dedupe guard (`apsLoadInFlight = tabId:urn`, cleared on all three completion paths); (c) `switchTab` back to an APS tab whose model is already on screen does resize+invalidate instead of unload+reload, and `closeTab` of the current APS tab now unloads the model from the singleton viewer immediately (previously it stayed resident until the next APS load evicted it).
...
- **Verbose logging toggle** in Settings — session-only (`Logger.VerboseLogging`, plain in-memory bool): OFF at every launch, applies instantly on flip, never persisted. When OFF, `apsDiag` messages whose body starts with `nav.` or `vrb.` are dropped at the bridge. Diagnostic dumps (`fit:`, `fit.frame:`, watchdog, load milestones, APS HTTP, app lifecycle, file/tab/settings, `app.memory` warnings) are always logged. The VM change-handler no-ops when the incoming value already equals the flag, so opening the Settings page (LoadAsync mirroring) doesn't log a phantom "verbose logging ON".
